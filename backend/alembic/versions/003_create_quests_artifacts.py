...
def upgrade() -> None:
    # Quest states enum
    quest_state_enum = sa.Enum(
        "LOCKED", "AVAILABLE", "IN_PROGRESS", "EVALUATING",
        "COMPLETED", "FAILED_ATTEMPT",
        name="quest_state",
    )
    quest_state_enum.create(op.get_bind(), checkfirst=True)

    # Evaluation type enum
    eval_type_enum = sa.Enum(
        "text_answer", "url_check", "quiz", "command_output",
    ...
    op.create_table(
        ...
        sa.Column("evaluation_type", sa.Enum("text_answer", "url_check", "quiz", "command_output", name="evaluation_type", create_type=False), nullable=False),
        ...
        sa.Column("required_artifact_ids", ARRAY(UUID(as_uuid=True)), server_default="{}"),
    ...
    op.create_table(
        ...
        sa.Column("state", sa.Enum("LOCKED", "AVAILABLE", "IN_PROGRESS", "EVALUATING", "COMPLETED", "FAILED_ATTEMPT", name="quest_state", create_type=False), server_default="LOCKED", nullable=False),
...
def downgrade() -> None:
    ...
    sa.Enum(name="evaluation_type").drop(op.get_bind(), checkfirst=True)
    sa.Enum(name="quest_state").drop(op.get_bind(), checkfirst=True)
