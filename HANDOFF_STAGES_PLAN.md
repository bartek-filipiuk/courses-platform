...
### Taski:
- [ ] T0: `.env.example` — plik z komentarzami: DATABASE_URL, GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET, JWT_SECRET_KEY, JWT_REFRESH_SECRET, OPENROUTER_API_KEY, REDIS_URL. Komentarz przy każdej zmiennej. (test → kod → verify)
...
- [ ] T5: Auth backend — OAuth callback (GitHub), JWT creation (access 15min + refresh 7d), refresh token rotation (stary invalidated), `POST /api/auth/logout` (token blacklist w Redis, TTL = remaining JWT lifetime), middleware `get_current_user` (JWT lub API Key) (test → kod → verify)
