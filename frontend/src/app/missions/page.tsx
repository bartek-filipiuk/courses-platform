	...
	if (loading) {
		return (
			<div className="min-h-screen bg-bg-base p-8">
				<h1 className="text-3xl font-bold text-text-primary mb-8">Missions</h1>
	...
	return (
		<div className="min-h-screen bg-bg-base p-8">
			<div className="max-w-7xl mx-auto">
				<FadeInUp>
				<h1 className="text-3xl font-bold text-text-primary mb-2">Available Missions</h1>
				<p className="text-text-secondary mb-8">Choose your operation and begin the quest.</p>
				</FadeInUp>

				{courses.length === 0 ? (
					<div className="text-center py-20">
						<p className="text-text-secondary text-lg">No missions available yet.</p>
					</div>
				) : (
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
						{courses.map((course) => (
							<Link key={course.id} href={`/missions/${course.id}`}>
								<GlassCard variant="interactive" className="group">
									...
									{course.narrative_title && (
										<p className="text-xs font-mono text-accent-primary uppercase tracking-wider mb-2">
											{course.narrative_title}
										</p>
									)}
									...
									<div className="flex items-center justify-between mt-auto pt-4 border-t border-border-default">
										{course.persona_name && (
											<span className="text-xs font-mono text-accent-info">
